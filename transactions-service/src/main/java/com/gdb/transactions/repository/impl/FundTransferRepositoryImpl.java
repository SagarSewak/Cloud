...
public class FundTransferRepositoryImpl implements FundTransferRepository {
    ...
    private FundTransfer insert(FundTransfer fundTransfer) {
        String sql = """
            ...
            VALUES (:fromAccount, :toAccount, :transferAmount, :transferMode)
            """;

        MapSqlParameterSource params = new MapSqlParameterSource()
                .addValue("fromAccount", fundTransfer.getFromAccount())
                .addValue("toAccount", fundTransfer.getToAccount())
                .addValue("transferAmount", fundTransfer.getTransferAmount())
    ...
    private FundTransfer update(FundTransfer fundTransfer) {
        String sql = """
            ...
            SET from_account = :fromAccount, to_account = :toAccount, 
                transfer_amount = :transferAmount, transfer_mode = :transferMode
            WHERE id = :id
            """;
    ...
    public Optional<FundTransfer> findById(Long id) {
        ...
        List<FundTransfer> results = jdbcTemplate.query(sql, params, rowMapper);
    ...
    public BigDecimal getDailyTransferAmount(Long accountNumber, LocalDate date) {
        String sql = """
            ...
            FROM fund_transfers
            WHERE from_account = :accountNumber AND DATE(created_at) = :date
            """;
    ...
    public List<FundTransfer> findByAccount(Long accountNumber, int limit, int offset) {
        ...
        // Fixed MOD3-BUG-01 by using named parameters which correctly handle multiple occurrences of the same variable.
        MapSqlParameterSource params = new MapSqlParameterSource()
                .addValue("accountNumber", accountNumber)
